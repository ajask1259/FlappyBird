...
class GameScene: SKScene,SKPhysicsContactDelegate {
    ...
    func setupCoin(){
        ...
        // 画面外まで移動するアクションを作成
        let moveCoin = SKAction.moveBy(x: -movingDistance*2, y: 0, duration:4*2)
        ...
        let createCoinAnimation = SKAction.run({
            ...
            let random_y = CGFloat.random(in: self.frame.size.height/5..<self.frame.size.height/1.2)
            // コイン配置
            let coin = SKSpriteNode(texture: coinTexture)
            coin.position = CGPoint(x:self.frame.size.width+125, y: random_y)
            coin.zPosition = -50 // 雲より手前、地面より奥
        ...
        // 次のcoin作成までの時間待ちのアクションを作成
        let waitAnimation = SKAction.wait(forDuration: 2)

        // coinを作成->時間待ち->coinを作成を無限に繰り返すアクションを作成
